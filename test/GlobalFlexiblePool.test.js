...
describe("Global Flexible Pool", function () {
  let staking
  let erc20Mock
  let owner, user
  ...
  before(async() => {
    ...
    const globalEpoch = (await deployContract("GlobalEpoch", [epoch1Start]))
  ...
  describe("Deposit", function () {
    ...
    it("Reverts if amount > allowance", async function () {
      ...
      ).to.be.revertedWith("deposit: Token allowance too small!")
    })
  ...
  describe("Withdraw", function () {
    it("Reverts if user has no balance", async function () {
      ...
      ).to.be.revertedWith("withdraw: Balance too small!")
    })
  ...
  describe("getEpochPoolSize", function () {
    ...
    it("Reverts if there's a gap", async function () {
      ...
      await expect(deposit(user, amount)).to.be.revertedWith("manualEpochInit: previous epoch not initialized!")
    })
  ...
  async function getEpochUserBalance(addr, epochId) {
